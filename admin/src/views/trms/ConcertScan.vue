...
const alreadyAttended = ref(false)
...
async function lookup(payload) {
  ...
  try {
    const response = await trmsStore.scanConcertRegistration(payload)
    result.value = response.data
    alreadyAttended.value = response.already_attended === true
    state.value = 'found'
    pushHistory(response.data, alreadyAttended.value)
    if (mode.value === 'camera') {
      showScanResultModal.value = true
    }
  } catch (err) {
    state.value = 'error'
    errorMessage.value = err.message || 'Registration not found.'
...
function pushHistory(record, duplicate = false) {
  history.value = [
...
async function startCamera() {
  ...
  try {
    ...
    if (err.name === 'NotAllowedError') {
      ...
      cameraError.value = 'Unable to open camera: ' + (err.message || err.name)
    }
  }
}
...
function scanLoop() {
  ...
  if (video.readyState === video.HAVE_ENOUGH_DATA) {
    ...
    if (code && code.data && !scanning.value && state.value !== 'found') {
      stopCamera()
      lastScannedCode.value = code.data
      lookup({ qr_code: code.data })
      return
    }
  }
...
function closeScanResultModal() {
  showScanResultModal.value = false
  state.value = 'idle'
}
...
async function downloadTicket(id) {
  try {
    ...
    errorMessage.value = err.message || 'Failed to download ticket PDF.'
  }
}
...
<template>
    ...
    <div class="fade-in-up">
      ...
      <div class="row g-4 align-items-center">
        <div class="col-lg-7">
          ...
          <h1 class="h3 fw-bold mb-3 text-champagne">Scan Registration</h1>
          <p class="lead text-champagne-muted mb-0">
            Verify audience registrations by scanning the QR code on their ticket or entering the
        ...
        <div class="content-card bg-dark h-100">
          ...
          <div v-if="mode === 'camera'">
            <div class="scanner-wrapper mb-3 position-relative rounded overflow-hidden bg-dark">
              ...
              <!-- Scan overlay frame -->
              <div
                class="scan-overlay position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center pointer-events-none"
              >
              ...
              <div
                ...
                class="position-absolute bottom-0 start-0 w-100 text-center pb-2"
              >
            ...
            <form @submit.prevent="submitManual">
              <div class="mb-3">
                ...
                <div class="input-group">
                  ...
                  <input
                    ...
                    placeholder="e.g. SDG_42_&hellip; or 42"
                    autocomplete="off"
                    autofocus
              ...
              <div class="d-grid">
                ...
                >
                  ...
                  {{ scanning ? 'Looking up&hellip;' : 'Look Up' }}
                </button>
              </div>
            </form>
          </div>
        ...
        <div class="content-card bg-dark h-100 d-flex align-items-center justify-content-center">
          <div class="py-5 text-center">
            ...
            <p class="mb-0 text-champagne-muted">
              Point your camera at a ticket's QR code or switch to manual entry.
            </p>
          </div>
        </div>
      </div>
          ...
          <div v-if="alreadyAttended" class="scan-modal-duplicate">
            ...
            <div>
              ...
              <div class="small opacity-75">
                This ticket was scanned at {{ formatDate(result.attended_at) }}.
              </div>
            </div>
          </div>
          ...
          <div class="scan-modal-details">
            ...
            <div class="scan-detail-row scan-detail-row--split">
              ...
              <div class="scan-detail-col">
                ...
                <div
                  class="scan-detail-value"
                  :class="result.attended_at ? 'text-success' : ''"
                >
  ...
  <Teleport to="body">
    <transition name="scan-modal">
      <div v-if="showErrorModal" class="scan-modal-overlay" @click.self="closeErrorModal">
        <div class="scan-modal-sheet" role="dialog" aria-modal="true" aria-label="Scan error">
          ...
          <div class="scan-modal-status">
            ...
            <div class="scan-status-text">
              ...
              <div class="scan-status-sub">
                This registration number or QR code is not recognized.
              </div>
            </div>
          </div>
          ...
          <div class="scan-modal-actions">
            <button type="button" class="btn btn-outline-gold flex-fill" @click="closeErrorModal">
...
.scan-modal-sheet {
  ...
  border-radius: 16px;
  border: 1px solid rgba(234, 220, 194, 0.1);
  background: linear-gradient(145deg, rgba(26, 31, 48, 0.98), rgba(17, 20, 32, 0.98));
  box-shadow:
