                    ...
                    <div class="bg-dark text-white rounded p-4 h-100">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-white-50 small">{{ upcomingConcerts.length }} available schedule{{ upcomingConcerts.length === 1 ? '' : 's' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="content-card">
            ...
            <div v-else-if="upcomingConcerts.length" class="row g-3">
                <div class="col-md-6 col-xl-4" v-for="concert in upcomingConcerts" :key="concert.id">
                    <button class="concert-option w-100 text-start" type="button" @click="selectConcert(concert)">
                        ...
                        <p v-if="concert.description" class="small text-muted mt-3 mb-0">{{ concert.description }}</p>
                    </button>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        selectConcert(concert) {
            this.$router.push(`/trms/concert/registration/${slugifyTitle(concert.title)}`)
        },
        toDateKey(date) {
