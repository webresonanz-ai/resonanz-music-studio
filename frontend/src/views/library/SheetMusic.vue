<template>
  <div class="fade-in-up">
      ...
      <div class="row g-2">
        <div class="col-12 col-md-6">
          <div class="search-wrap">
            ...
            <input v-model="search" type="text" class="search-input" placeholder="Search by title, composer, or arranger…" />
    ...
    <template v-else>
      <div v-if="filteredScores.length === 0" class="empty-state content-card">
        <div class="empty-icon"><i class="bi bi-music-note"></i></div>
        <h5 class="mt-3 mb-1">No scores found</h5>
      ...
      <div v-if="viewMode === 'list'" class="scores-list content-card p-0">
      ...
      <transition name="modal">
        <div v-if="selectedScore && !showForm && !deleteTarget" class="modal-overlay" @click.self="selectedScore = null">
          <div class="modal-sheet" role="dialog" aria-modal="true">
            <button class="modal-close-btn" @click="selectedScore = null" aria-label="Close"><i class="bi bi-x-lg"></i></button>
            <div class="d-flex flex-column flex-md-row gap-4">
              ...
              <div class="flex-fill">
                ...
                <div v-if="isAdmin" class="d-flex gap-2 mt-3">
                  <button class="btn btn-sm btn-outline-primary" @click="openEdit(selectedScore)"><i class="bi bi-pencil me-1"></i>Edit</button>
        ...
        <div v-if="showForm" class="modal-overlay" @click.self="closeForm">
          <div class="modal-sheet" role="dialog" aria-modal="true">
            ...
            <div class="modal-header-row d-flex align-items-center gap-2 mb-3">
              ...
              <div><h5 class="mb-0">{{ isEditing ? 'Edit Score' : 'Add Score' }}</h5><p class="text-muted mb-0 small">{{ isEditing ? 'Update the score details' : 'Fill in the details below' }}</p></div>
            </div>
            <div v-if="formError" class="alert alert-danger py-2 small">{{ formError }}</div>
            <form @submit.prevent="submitForm">
              <div class="row g-3">
                <div class="col-12"><label class="form-label small fw-bold">Title <span class="text-danger">*</span></label><input v-model="form.title" type="text" class="form-control form-control-sm" placeholder="Score title" required /></div>
                <div class="col-12 col-sm-6"><label class="form-label small fw-bold">Composer</label><input v-model="form.composer" type="text" class="form-control form-control-sm" placeholder="Composer name" /></div>
                <div class="col-12 col-sm-6"><label class="form-label small fw-bold">Arranger</label><input v-model="form.arranger" type="text" class="form-control form-control-sm" placeholder="Arranger name" /></div>
                <div class="col-12 col-sm-4"><label class="form-label small fw-bold">Genre</label>
                ...
                <div class="col-12 col-sm-4"><label class="form-label small fw-bold">Difficulty</label>
                  <select v-model="form.difficulty" class="form-select form-select-sm">
                    <option value="Beginner">Beginner</option><option value="Intermediate">Intermediate</option><option value="Advanced">Advanced</option>
                  </select>
                </div>
                <div class="col-12 col-sm-4"><label class="form-label small fw-bold">Pages</label><input v-model.number="form.pages" type="number" min="0" class="form-control form-control-sm" /></div>
                <div class="col-12"><label class="form-label small fw-bold">Thumbnail URL</label><input v-model="form.thumbnail" type="url" class="form-control form-control-sm" placeholder="https://…" /></div>
                <div v-if="isAdmin" class="col-12">
                  <label class="form-label small fw-bold">Upload PDF</label>
                  <input ref="pdfInput" type="file" accept=".pdf,application/pdf" class="form-control form-control-sm" @change="onPdfSelect" :disabled="uploadingPdf" />
                  <div v-if="uploadingPdf" class="mt-1 small text-muted d-flex align-items-center gap-1"><span class="spinner-border spinner-border-sm"></span>Uploading PDF…</div>
                  ...
                  <div v-else-if="pendingPdfFile" class="mt-1 small text-muted d-flex align-items-center gap-1"><i class="bi bi-paperclip"></i>{{ pendingPdfFile.name }} <span class="badge bg-warning text-dark">will upload on save</span></div>
                </div>
              </div>
      ...
      <transition name="modal">
        <div v-if="deleteTarget" class="modal-overlay" @click.self="cancelDelete">
          <div class="modal-sheet modal-sheet--sm text-center" role="dialog" aria-modal="true">
            ...
            <h5 class="mt-3 mb-1">Delete Score</h5>
            <p class="text-muted mb-4 small">Remove <strong>{{ deleteTarget.title }}</strong>? This cannot be undone.</p>
...
.sheet-title { font-size:clamp(1.4rem,4vw,2rem);font-weight:800;color:var(--ink-color);letter-spacing:-.01em }
...
.filter-chip { display:inline-flex;align-items:center;gap:.35rem;padding:.3rem .85rem;border:1px solid var(--hairline-color);border-radius:999px;background:rgba(255,253,248,.8);color:var(--muted-color);font-size:.8rem;font-weight:600;cursor:pointer;transition:all .18s ease;user-select:none }
.filter-chip:hover { border-color:var(--gold-color);color:var(--ink-color);background:rgba(200,164,93,.1) }
.filter-chip.active { border-color:var(--gold-color);background:rgba(200,164,93,.15);color:var(--ink-color) }

/* ── States ── */
.loading-state { display:flex;flex-direction:column;align-items:center;justify-content:center;padding:5rem 1rem;color:var(--muted-color);gap:1rem }
...
.score-card { border-radius:var(--radius-md);border:1px solid var(--hairline-color);background:rgba(255,253,248,.96);box-shadow:0 2px 12px rgba(19,18,16,.06);overflow:hidden;display:flex;flex-direction:column;animation:fadeInUp .4s ease-out both;cursor:pointer;transition:transform .22s ease,box-shadow .22s ease,border-color .22s ease }
.score-card:active { transform:translateY(-1px) }
@media (hover:hover) {
  .score-card:hover { transform:translateY(-4px);box-shadow:0 12px 32px rgba(19,18,16,.13);border-color:rgba(200,164,93,.4) }
}

.score-thumb-wrap { position:relative;height:180px;overflow:hidden;background:#ede8df;flex-shrink:0 }
...
.score-title { margin:0;font-size:.92rem;font-weight:700;color:var(--ink-color);white-space:nowrap;overflow:hidden;text-overflow:ellipsis }
.score-composer { font-size:.78rem;color:var(--muted-color);margin:.15rem 0 .6rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis }
.score-meta { display:flex;gap:.6rem;flex-wrap:wrap }
.score-meta-item { display:inline-flex;align-items:center;gap:.25rem;font-size:.73rem;color:var(--muted-color) }
.score-meta-item i { font-size:.7rem }
.score-actions { display:flex;border-top:1px solid var(--hairline-color);padding:.45rem .65rem;gap:.35rem }

/* ── List ── */
.scores-list { overflow:hidden }
.list-header { display:grid;grid-template-columns:48px 2fr 1.2fr 1.2fr 1fr .9fr .6fr;gap:.5rem;padding:.75rem 1rem;border-bottom:1px solid var(--hairline-color);font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--muted-color) }
...
.list-title { font-weight:600;font-size:.9rem;color:var(--ink-color);white-space:nowrap;overflow:hidden;text-overflow:ellipsis }
.list-muted { font-size:.82rem;color:var(--muted-color);white-space:nowrap;overflow:hidden;text-overflow:ellipsis }
...
.action-btn { flex:1;border:1px solid var(--hairline-color);border-radius:6px;background:transparent;color:var(--muted-color);font-size:.82rem;padding:.32rem;cursor:pointer;transition:all .18s ease;line-height:1;display:inline-flex;align-items:center;justify-content:center }
...
.modal-sheet { position:relative;background:var(--surface-color,#fffdf8);border-radius:14px;border:1px solid var(--hairline-color);box-shadow:0 32px 72px rgba(10,10,15,.36),0 0 0 1px rgba(200,164,93,.1);width:100%;max-width:640px;padding:1.25rem;margin:auto }
...
.modal-close-btn { position:absolute;top:.75rem;right:.75rem;border:0;background:rgba(34,29,20,.08);color:var(--muted-color);width:34px;height:34px;border-radius:8px;display:grid;place-items:center;font-size:.85rem;cursor:pointer;transition:background .18s,color .18s }
...
.detail-title { font-size:clamp(1.1rem,3vw,1.35rem);font-weight:800;color:var(--ink-color);margin-bottom:1rem;line-height:1.25 }
...
.df-label { font-size:.78rem;font-weight:600;color:var(--muted-color);min-width:80px;flex-shrink:0 }
.df-value { font-size:.9rem;color:var(--ink-color);word-break:break-word }
.pdf-link { color:var(--accent-color);text-decoration:none;display:inline-flex;align-items:center;gap:.25rem;font-weight:600;transition:color .15s }
.pdf-link:hover { color:var(--gold-color);text-decoration:underline }
