  ...
  <div class="fade-in-up">
    <div class="content-card bg-dark mb-4">
      <div class="d-flex align-items-start justify-content-between gap-3 flex-wrap">
        ...
        <div class="d-flex gap-2 align-items-center">
          <button
            class="btn btn-outline-gold btn-sm d-flex align-items-center gap-1 position-relative"
            @click="showCart = true"
          >
    ...
    <template v-else>
      ...
      <div v-if="viewMode === 'grid'" class="scores-grid">
        <article
          v-for="(score, idx) in filteredScores"
          :key="score.id"
          class="score-card"
        ...
        >
          <div class="score-thumb-wrap">
            <img
              :src="score.thumbnail || defaultThumb"
              :alt="score.title"
              class="score-thumb"
              loading="lazy"
            />
            ...
            <span v-if="score.price > 0" class="price-badge"
              >Rp {{ formatPrice(score.price) }}</span
            >
          </div>
          <div class="score-body">
            ...
            <div class="score-meta">
              ...
              <span class="score-meta-item"
                ><i class="bi bi-file-earmark-text"></i>{{ score.pages }} pp</span
              >
            </div>
            <button
              v-if="score.price > 0"
              class="btn btn-gold btn-sm w-100 mt-2 add-to-cart-btn"
              @click.stop="addToCart(score)"
            >
              <i class="bi bi-cart-plus me-1"></i
              >{{ isInCart(score.id) ? "In Cart" : "Add to Cart" }}
            </button>
          </div>
          <div v-if="isAdmin" class="score-actions" @click.stop>
            <button class="action-btn action-edit" title="Edit" @click="openEdit(score)">
              <i class="bi bi-pencil"></i>
            </button>
            <button class="action-btn action-delete" title="Delete" @click="confirmDelete(score)">
      ...
      <div v-if="viewMode === 'list'" class="scores-list content-card bg-dark p-0">
        <div class="list-header">
          <span class="lh-thumb"></span><span class="lh-title">Title</span
          ><span class="lh-composer">Composer</span> <span class="lh-arranger">Arranger</span
          ><span class="lh-genre">Genre</span><span class="lh-difficulty">Level</span>
        ...
        >
          ...
          /></span>
          <span class="lh-title"
            ><span class="list-title">{{ score.title }}</span></span
          >
          <span class="lh-composer list-muted">{{ score.composer }}</span>
          <span class="lh-arranger list-muted">{{ score.arranger }}</span>
          <span class="lh-genre"
            ><span class="genre-pill">{{ score.genre }}</span></span
          >
          <span class="lh-difficulty"
            ><span class="diff-badge" :class="diffClass(score.difficulty)">{{
              score.difficulty
            }}</span></span
          >
          <span class="lh-price"
            ><span class="price-value">{{
              score.price > 0 ? "Rp " + formatPrice(score.price) : "FREE"
            }}</span></span
          >
    ...
    <Teleport to="body">
      <transition name="modal">
        <div
          v-if="selectedScore && !showForm && !deleteTarget"
          class="modal-overlay"
          @click.self="selectedScore = null"
        >
          <div class="modal-sheet modal-sheet-dark" role="dialog" aria-modal="true">
            <button
              class="modal-close-btn modal-close-btn-dark"
            ...
            <div class="d-flex flex-column flex-md-row gap-4">
              <div class="detail-thumb-wrap">
                <img
                  :src="selectedScore.thumbnail || defaultThumb"
                  :alt="selectedScore.title"
                  class="detail-thumb"
                />
              </div>
              <div class="flex-fill">
                ...
                <div class="detail-field">
                  <span class="df-label">Composer</span
                  ><span class="df-value">{{ selectedScore.composer }}</span>
                </div>
                <div class="detail-field">
                  <span class="df-label">Arranger</span
                  ><span class="df-value">{{ selectedScore.arranger || "–" }}</span>
                </div>
                <div class="detail-field">
                  <span class="df-label">Genre</span
                  ><span class="df-value">{{ selectedScore.genre }}</span>
                </div>
                <div class="detail-field">
                  <span class="df-label">Difficulty</span
                  ><span class="df-value"
                    ><span class="diff-badge" :class="diffClass(selectedScore.difficulty)">{{
                      selectedScore.difficulty
                    }}</span></span
                  >
                </div>
                <div class="detail-field">
                  <span class="df-label">Pages</span
                  ><span class="df-value">{{ selectedScore.pages }}</span>
                </div>
                <div class="detail-field">
                  <span class="df-label">Price</span
                  ><span class="df-value"
                    ><strong class="price-highlight">{{
                      selectedScore.price > 0 ? "Rp " + formatPrice(selectedScore.price) : "FREE"
                    }}</strong></span
                  >
                </div>
                <div class="d-flex gap-2 mt-3 flex-wrap">
                  <button
                    v-if="selectedScore.price > 0 && !isInCart(selectedScore.id)"
                    class="btn btn-gold btn-sm"
                    @click.stop="addToCart(selectedScore)"
                  >
                    <i class="bi bi-cart-plus me-1"></i>Add to Cart
                  </button>
                  <button
                    v-if="isInCart(selectedScore.id)"
                    class="btn btn-sm btn-outline-danger"
                    @click.stop="removeFromCart(selectedScore.id)"
                  >
                    <i class="bi bi-cart-dash me-1"></i>Remove
                  </button>
                  <button
                    v-if="selectedScore.file_url"
                    class="btn btn-outline-gold btn-sm"
                    @click.stop="openPdfPreview(selectedScore.file_url)"
                  >
                    <i class="bi bi-eye me-1"></i>Preview
                  </button>
                  <button
                    v-if="authStore.token && isInCart(selectedScore.id)"
                    class="btn btn-gold btn-sm"
                    @click.stop="closeDetailAndCheckout"
                  >
                    <i class="bi bi-credit-card me-1"></i>Checkout
                  </button>
                </div>
                <div v-if="isAdmin" class="d-flex gap-2 mt-2">
                  <button class="btn btn-sm btn-outline-gold" @click="openEdit(selectedScore)">
                    <i class="bi bi-pencil me-1"></i>Edit
                  </button>
                  <button
                    class="btn btn-sm btn-outline-danger"
                    @click="confirmDelete(selectedScore)"
                  >
    ...
    <Teleport to="body">
      <transition name="modal">
        <div v-if="showForm" class="modal-overlay" @click.self="closeForm">
          <div
            class="modal-sheet modal-sheet-dark modal-sheet--form"
            role="dialog"
            aria-modal="true"
          >
            <button
            ...
            <div class="modal-header-row d-flex align-items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p class="form-modal-subtitle mb-0">
                  {{
                    isEditing
                      ? "Update the score details below"
                      : "Fill in the details for the new score"
                  }}
                </p>
              </div>
            </div>
            ...
            <form @submit.prevent="submitForm" class="score-form">
                  ...
                  <div class="col-12 col-md-6">
                    ...
                    <div class="input-icon-wrap">
                      ...
                      <input
                        ...
                        type="text"
                        class="form-input"
                        placeholder="Arranger name (optional)"
                      />
                    </div>
                  </div>
                </div>
              ...
              <div class="form-section">
                ...
                <div class="row g-3">
                  ...
                  <div v-if="isAdmin" class="col-12">
                    ...
                    <div
                      class="pdf-dropzone"
                      :class="{ 'has-pdf': form.file_url || pendingPdfFile }"
              ...
              <div class="form-footer">
                <button type="button" class="btn btn-outline-gold" @click="closeForm">
                  Cancel
                </button>
    ...
    <Teleport to="body">
      <transition name="modal">
        <div v-if="deleteTarget" class="modal-overlay" @click.self="cancelDelete">
          <div
            class="modal-sheet modal-sheet-dark modal-sheet--sm text-center"
            role="dialog"
            aria-modal="true"
          >
            ...
            <p class="text-champagne-muted mb-4 small">
              Remove
              <strong style="color: rgba(234, 220, 194, 0.92)">{{ deleteTarget.title }}</strong
            ...
            <div v-else class="cart-items">
              <div v-for="item in cartStore.items" :key="item.score_id" class="cart-item">
                ...
                <button
                  ...
                  @click="cartStore.removeItem(item.score_id)"
                  title="Remove"
                >
                  <i class="bi bi-trash3"></i>
                </button>
              ...
              <div class="cart-checkout">
                ...
                <textarea
                  ...
                  class="form-control-dark form-control-sm mb-2"
                  rows="2"
    ...
    <Teleport to="body">
      <transition name="modal">
        <div v-if="showOrders" class="modal-overlay" @click.self="showOrders = false">
          <div
            class="modal-sheet modal-sheet-dark modal-sheet--orders"
            role="dialog"
            aria-modal="true"
          >
            <button
            ...
            <div class="modal-header-row d-flex align-items-center gap-2 mb-3">
              ...
              <div>
                <h5 class="mb-0 text-champagne">My Orders</h5>
                <p class="text-champagne-muted mb-0 small">Your purchase history</p>
              </div>
            </div>
      ...
      <transition name="modal">
        <div v-if="checkoutResult" class="modal-overlay" @click.self="checkoutResult = null">
          ...
          >
            ...
            <p class="text-champagne-muted small">
              Total:
              <strong class="price-highlight"
                >Rp {{ formatPrice(checkoutResult.total_amount) }}</strong
              >
            </p>
            <p class="text-champagne-muted small mb-3">
              Status:
              <span class="order-status order-status--pending_payment">Pending Payment</span>
            </p>
            <div class="d-flex gap-2 justify-content-center">
              <button
                class="btn btn-outline-gold btn-sm"
              ...
              <button
                ...
                @click="
                  checkoutResult = null;
                  showSnapPayment(checkoutResult.id);
                "
              >
    ...
    <Teleport to="body">
      <transition name="modal">
        <div
          v-if="showPdfPreview"
          class="modal-overlay pdf-preview-overlay"
          ...
          <div class="pdf-preview-modal">
            ...
            <div class="pdf-preview-body">
              <iframe
                ...
                allowfullscreen
              ></iframe>
              <div class="pdf-watermark-overlay" aria-hidden="true" :style="watermarkBgStyle"></div>
              <div class="pdf-click-guard" @contextmenu.prevent></div>
            </div>
          </div>
        </div>
      </transition>
...
const defaultThumb =
  "data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 200 260%22%3E%3Crect fill=%22%23f5f0e8%22 width=%22200%22 height=%22260%22/%3E%3Ctext x=%22100%22 y=%22140%22 text-anchor=%22middle%22 fill=%22%23c8a45d%22 font-size=%2248%22 font-family=%22serif%22%3E%E2%99%AA%3C/text%3E%3C/svg%3E";
...
export default {
  ...
  setup() {
    ...
    const diffClass = (d) => ({
      "diff-beginner": d === "Beginner",
      "diff-intermediate": d === "Intermediate",
      "diff-advanced": d === "Advanced",
    });
    ...
    const watermarkSvg = `data:image/svg+xml,${encodeURIComponent(
      `<svg xmlns="http://www.w3.org/2000/svg" width="280" height="280" viewBox="0 0 280 280">
        <text x="140" y="140" transform="rotate(-30 140 140)" fill="rgba(0,0,0,0.65)" font-family="Arial,Helvetica,sans-serif" font-size="30" font-weight="bold" text-anchor="middle">FOR PREVIEW ONLY</text>
      </svg>`,
    )}`;
    ...
    const onPdfSelect = (e) => {
      ...
      if (editingId.value) {
        ...
        store
          .uploadScorePdf(editingId.value, file)
          .then((r) => {
            if (r?.data?.file_url) form.value.file_url = r.data.file_url;
          })
          .catch((err) => {
            formError.value = err.message || "PDF upload failed";
          })
          .finally(() => {
            uploadingPdf.value = false;
            if (pdfInput.value) pdfInput.value.value = "";
    ...
    const doDelete = async () => {
      deleting.value = true;
      try {
        await store.deleteScore(deleteTarget.value.id);
        cancelDelete();
      } catch (err) {
...
@media (hover: hover) {
  .score-card:hover .score-thumb-overlay {
    opacity: 1;
  }
}
.score-badge {
...
/* ── List ── */
.scores-list.content-card.bg-dark {
  overflow: hidden;
  color: rgba(234, 220, 194, 0.78);
}
.list-header {
  display: grid;
  grid-template-columns: 48px 2fr 1.2fr 1.2fr 1fr 0.9fr 0.8fr;
  gap: 0.5rem;
...
@media (max-width: 767.98px) {
  .list-header {
    grid-template-columns: 40px 2fr 1fr 0.7fr 0.7fr;
    font-size: 0.65rem;
...
.cart-panel {
  ...
  max-width: 400px;
  height: 100%;
  background: linear-gradient(180deg, rgba(26, 31, 48, 0.99), rgba(17, 20, 32, 0.99));
...
.pdf-dropzone {
  position: relative;
  border: 2px dashed rgba(234, 220, 194, 0.12);
  border-radius: 10px;
  padding: 1rem;
  cursor: pointer;
  transition:
...
.pdf-dropzone-link:hover {
  text-decoration: underline;
}
.pdf-dropzone-replace {
