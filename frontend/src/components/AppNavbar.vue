<template>
    <nav class="main-navbar" role="navigation" aria-label="Main navigation">
                ...
                <div class="nav-user" :class="{ 'is-open': userMenuOpen }">
                    ...
                    <ul class="nav-dropdown nav-dropdown-end" role="menu">
                        ...
                        <li role="none">
                            ...
                            >
                                <span class="dropdown-link-icon">
                                    <i :class="authStore.user ? 'bi bi-box-arrow-right' : 'bi bi-box-arrow-in-right'"></i>
                                </span>
        ...
        <!-- Mobile nav menu -->
        <div class="navbar-mobile-menu" :class="{ 'is-open': mobileMenuOpen }" aria-hidden="!mobileMenuOpen">
            <template v-for="item in filteredNavItems" :key="item.label">
                <div v-if="item.children" class="mobile-nav-group">
                    ...
                    >
                        ...
                        <span>{{ item.label }}</span>
                        <i class="bi bi-chevron-down mobile-chevron" :class="{ rotated: openMobileGroup === item.label }"></i>
                    </button>
...
export default {
    ...
    setup() {
        ...
        const onDropdownLeave = () => {
            if (!isDesktop()) return
            leaveTimer = setTimeout(() => { openDropdown.value = null }, 120)
        }
        ...
        return {
            navigationStore, authStore, router,
            filteredNavItems,
            primaryNavItems, overflowNavItems,
            openDropdown, openMobileGroup,
            userMenuOpen, mobileMenuOpen,
            searchFocused, searchQuery,
            onDropdownEnter, onDropdownLeave,
            toggleDropdown, closeDropdown,
            toggleMobileGroup, closeMobileMenu,
            getUserInitials, logout
        }
    }
}
</script>
...
.nav-search-wrap {
    ...
    border-radius: 8px;
    background: rgba(234, 220, 194, 0.06);
    transition: width 0.28s ease, border-color 0.2s, background 0.2s;
}
...
.mobile-nav-group-header { justify-content: flex-start; }
...
.mobile-nav-children.is-open { max-height: 400px; }
...
@media (max-width: 991.98px) {
    ...
    .navbar-overflow-row { display: none; }
    .nav-mobile-toggle { display: flex; }
    .navbar-mobile-menu { display: flex; }
    .brand-sub { display: none; }
    .brand-sep { display: none; }
    .nav-search-wrap { width: 130px; }
    .nav-search-wrap.is-focused { width: 170px; }
}
...
@media (max-width: 575.98px) {
    .navbar-inner { padding: 0.5rem 0.65rem; }
